...
public class UserController : Controller
{
    ...
    public ActionResult Search(string query)
    {
        ...
        {
            return View("Index", userlist); // Return all users if the query is empty
        }

        // Find users whose names contain the query (case-insensitive)
        var results = userlist.Where(u => u.Name.Contains(query, StringComparison.OrdinalIgnoreCase)).ToList();

        return View("Index", results); // Return the results to the Index view
    }
}
